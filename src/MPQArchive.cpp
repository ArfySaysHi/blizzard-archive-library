...
MPQArchive::MPQArchive(std::string const &path, Locale locale,
  ...
  if (SFileOpenFileEx(_handle, "(listfile)", 0, &fh)) {
    ...
    SFileReadFile(fh, readbuffer.data(), static_cast<DWORD>(filesize), nullptr,
                  nullptr);
    SFileCloseFile(fh);
...
bool MPQArchive::openFile(Listfile::FileKey const &file_key, Locale locale,
                          HANDLE *file_handle) const {
  assert(file_key.hasFilepath());
  return SFileOpenFileEx(
      _handle, ClientData::normalizeFilenameWoW(file_key.filepath()).c_str(), 0,
      file_handle);
}

bool BlizzardArchive::Archive::MPQArchive::writeFile(
    Listfile::FileKey const &file_key, const char *file_data,
    std::size_t buf_size, Locale locale, unsigned long skip_error,
    bool compress) {
  // archive must have been open in non STREAM_FLAG_READ_ONLY mode or
  // SFileCreateFile will return ERROR_ACCESS_DENIED
  ...
  if (compress) {
    dwFlags |= MPQ_FILE_COMPRESS; // Blizzard seems to use MPQ_FILE_SECTOR_CRC.
                                  // // MPQ_FILE_ENCRYPTED
    dwCompression |=
        MPQ_COMPRESSION_ZLIB; // MPQ_COMPRESSION_BZIP2 is super slow
  }
  ...
  if (SFileCreateFile(_handle, file_key.filepath().c_str(), 0, buf_size, 0,
    ...
    if (skip_error == error) {
      ...
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::writeFile() SFileCreateFile: Error creating file: " +
              file_key.filepath() + "in archive" + _path +
    ...
    if (error == ERROR_DISK_FULL) {
      ...
      if (!SFileRemoveFile(_handle, file_key.filepath().c_str(), 0))
        throw Exceptions::Archive::FileWriteFailedError(
            "MPQArchive::writeFile() SFileRemoveFile: Error replacing file: " +
                file_key.filepath() + "in archive" + _path,
            error);
      ...
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::writeFile() SFileCreateFile: Error creating file: " +
              file_key.filepath() + "in archive" + _path +
              "\nMPQ Archive ACCESS DENIED, make sure no other application is "
              "using this MPQ.",
          error);
    } else {
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::writeFile() SFileCreateFile: Error creating file: " +
              file_key.filepath() + "in archive" + _path + ", unhandled error",
          error);
    }
  }
  return true;
...
bool BlizzardArchive::Archive::MPQArchive::addFile(
    Listfile::FileKey const &wow_path, std::string file_disk_path,
    Locale locale, unsigned long skip_error, bool compress) {
  // archive must have been open in non STREAM_FLAG_READ_ONLY mode or
  // SFileCreateFile will return ERROR_ACCESS_DENIED

  // SFileSetAddFileCallback(_handle, AddFileCallback, pLogger);
  // SFileAddFileEx()
  // https://github.com/ladislav-zezula/StormLib/blob/master/src/SFileAddFile.cpp#L896
  // if (!SFileAddFileEx(_handle, disk_path, file_key.filepath().c_str(),
  ...
  if (compress) {
    dwFlags |=
        MPQ_FILE_COMPRESS |
        MPQ_FILE_SECTOR_CRC; // Blizzard seems to use MPQ_FILE_SECTOR_CRC. //
                             // MPQ_FILE_ENCRYPTED support ?
    dwCompression |=
        MPQ_COMPRESSION_ZLIB; // MPQ_COMPRESSION_BZIP2 is super slow
  }
  ...
  if (has_file)
    ...
    if (!SFileRemoveFile(_handle, wow_path.filepath().c_str(), 0))
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::addFile() SFileRemoveFile: Error replacing file: " +
          wow_path.filepath() + "in archive" + _path);

  if (SFileAddFileEx(_handle, file_disk_path.c_str(),
                     wow_path.filepath().c_str(), dwFlags, dwCompression,
                     MPQ_COMPRESSION_NEXT_SAME)) {
    _listfile->addFile(wow_path.filepath());

    // auto dwVerifyResult = SFileVerifyFile(_handle,
    ...
    if (skip_error == error) {
      // don't allow chain crashing on the same error
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::addFile() SFileAddFileEx: Error adding file: " +
              wow_path.filepath() + "to archive" + _path +
              ", got the same error twice.",
          error);
    }
    ...
    if (error == ERROR_DISK_FULL) {
      ...
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::addFile() SFileAddFileEx: Error creating file: " +
              wow_path.filepath() + "in archive" + _path +
              "\nMPQ Archive ACCESS DENIED, make sure no other application is "
              "using this MPQ.",
          error);
      ...
      throw Exceptions::Archive::FileWriteFailedError(
          "MPQArchive::addFile() SFileAddFileEx: Error creating file: " +
              wow_path.filepath() + "in archive" + _path + ", unhandled error",
          error);
    }
  }
...
bool MPQArchive::readFile(HANDLE file_handle, char *buffer,
                          std::size_t buf_size) const {
  assert(file_handle);
  return SFileReadFile(file_handle, buffer, static_cast<DWORD>(buf_size),
                       nullptr, nullptr);
}
...
bool BlizzardArchive::Archive::MPQArchive::closeToReadOnly() {
  ...
  // don't need to read listfile again, should be added when adding files in
  // writeFile()
...
bool MPQArchive::exists(Listfile::FileKey const &file_key,
                        Locale locale) const {
  assert(file_key.hasFilepath());
  return SFileHasFile(
      _handle, ClientData::normalizeFilenameWoW(file_key.filepath()).c_str());
}
