...
ClientData::ClientData(std::string const &path, ClientVersion version,
                       Locale locale, std::string const &local_path)
    : _version(version), _open_mode(OpenMode::LOCAL),
      _storage_type((version > ClientVersion::WOTLK) ? StorageType::CASC
                                                     : StorageType::MPQ),
      _locale_mode(locale), _path(path),
...
ClientData::ClientData(std::string const &path,
                       std::string const &cdn_cache_path, ClientVersion version,
                       Locale locale, std::string const &local_path)
    : _version(version), _open_mode(OpenMode::REMOTE),
      _storage_type((version > ClientVersion::WOTLK) ? StorageType::CASC
                                                     : StorageType::MPQ),
      _locale_mode(locale), _path(path),
      _local_path(ClientData::normalizeFilenameUnix(local_path)),
  ...
  case StorageType::MPQ:
    throw Exceptions::Archive::ArchiveOpenError(
        "MPQ storage does not support online loading.");
    break;
  }
}
...
bool BlizzardArchive::ClientData::mpqArchiveExistsOnDisk(
...
std::optional<Archive::MPQArchive *>
BlizzardArchive::ClientData::getMPQArchive(std::string const &archive_name) {
  ...
  for (auto *archive_ptr : _archives) {
    ...
    if (archive_ptr->path() == mpq_path) {
      if (Archive::MPQArchive *mpqArchive_ptr =
              dynamic_cast<Archive::MPQArchive *>(archive_ptr)) {
  ...
  if (mpqArchiveExistsOnDisk(archive_name)) {
    Archive::MPQArchive *new_archive =
        new Archive::MPQArchive(mpq_path, _locale_mode, &_listfile);
    if (new_archive) {
...
std::optional<Archive::MPQArchive *>
BlizzardArchive::ClientData::tryCreateMPQArchive(
    std::string const &archive_name) {
  if (_storage_type != StorageType::MPQ)
    return std::nullopt;
...
bool BlizzardArchive::ClientData::isMPQNameValid(
    std::string const &archive_name, bool exclude_base_mpqs) {
  ...
  for (auto const &filename : ClientData::ArchiveNameTemplates) {
    ...
    if (mpq_path.find("{number}") != std::string::npos) {
      ...
      for (char j = '2'; j <= '9'; j++) {
        ...
        auto mpq_filename =
            fs::path(mpq_path).filename().replace_extension(".mpq").string();
...
void ClientData::loadMPQArchive(std::string const &mpq_path) {
  ...
  if (fs::is_directory(mpq_path)) {
    _archives.push_back(
        new Archive::DirectoryArchive(mpq_path, _locale_mode, &_listfile));
  } else {
    _archives.push_back(
        new Archive::MPQArchive(mpq_path, _locale_mode, &_listfile));
  }
...
void ClientData::initializeCASCStorage() {
  ...
  switch (_open_mode) {
  case OpenMode::LOCAL: {
    _archives.push_back(new Archive::CASCArchive(_path, "", _locale_mode,
                                                 _open_mode, &_listfile));
    break;
  }
  case OpenMode::REMOTE: {
    assert(_cdn_cache_path.has_value());
    _archives.push_back(new Archive::CASCArchive(
        _path, _cdn_cache_path.value(), _locale_mode, _open_mode, &_listfile));
    break;
  }
  }
}
...
void ClientData::validateLocale() {
  ...
  case StorageType::MPQ: {
    if (static_cast<int>(_locale_mode)) // manual locale
        [[unlikely]] {
      fs::path realmlist_path =
          fs::path(_path) / "Data" / locale_name() / "realmlist.wtf";

      if (!fs::exists(realmlist_path)) {
        throw Exceptions::Locale::LocaleNotFoundError(
            "Requested locale \"" + std::string(locale_name().data()) +
            "\" does not exist in the client directory."
            "Be sure, that there is one containing the file "
            "\"realmlist.wtf\".");
      }
      ...
      for (unsigned i = 0; i < ClientData::Locales.size(); ++i) {
        fs::path realmlist_path =
            fs::path(_path) / "Data" / ClientData::Locales[i] / "realmlist.wtf";
      ...
      throw Exceptions::Locale::LocaleNotFoundError(
          "Automatic locale detection failed. "
  ...
  case StorageType::CASC: {
    if (_locale_mode == Locale::AUTO) {
      throw Exceptions::Locale::IncorrectLocaleModeError(
          "Automatic locale detection is not"
...
bool ClientData::readFile(Listfile::FileKey const &file_key,
...
std::array<int, 2> BlizzardArchive::ClientData::saveLocalFilesToArchive(
    Archive::MPQArchive *archive, bool compress, bool compact) {
  ...
  try {
    ...
    for (const auto &entry : fs::recursive_directory_iterator(_local_path)) {
      if (entry.is_regular_file()) {
        ...
        // filter noggit files
        if (extension == ".noggitproj" || extension == ".json" ||
            extension == ".ini")
          continue;

        file_count++;

        // use SFileAddFileEx high level function which does everything, or read
        // manually
        bool full_write_file = false;

        if (!full_write_file) {
          // get internal Wow path
          auto wow_path = fs::relative(entry.path(), fs::path(_local_path))
                              .generic_string();

          try {
            // normalize path with filekey or nah ?
            bool success =
                archive->addFile(wow_path, entry.path().generic_string(),
                                 _locale_mode, 0, compress);
            if (success)
              file_success++;
            else
              continue;
          ...
          if (file_stream.is_open()) {
            ...
            auto wow_path = fs::relative(entry.path(), fs::path(_local_path))
                                .generic_string();
            try {
              bool success = archive->writeFile(wow_path, buffer, buf_size,
                                                _locale_mode, 0, compress);
              if (success)
                file_success++;
              else
            ...
            continue; // std::cerr << "Unable to open file: " << entry.path() <<
                      // std::endl;
          }
        }
      }
...
std::string ClientData::getDiskPath(Listfile::FileKey const &file_key) {
  ...
  if (file_key.hasFilepath()) {
    return (fs::path(_local_path) /
            ClientData::normalizeFilenameUnix(file_key.filepath()))
        .string();
  } else {
    ...
    if (!filepath.empty()) {
      return (fs::path(_local_path) /
              ClientData::normalizeFilenameUnix(filepath))
          .string();
    } else {
      return (fs::path(_local_path) / "unknown_files/" /
